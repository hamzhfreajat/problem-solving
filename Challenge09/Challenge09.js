...
const arrInc = arr => arr.map(item => item+10);;
...
//
// ------------------------
const roundDecimals = arr => arr.map(item => Math.round(item));
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    ...
    mouseArray.forEach(mouse => {
        keyBoardArray.forEach(keyboard => {
            let newPrice = mouse + keyboard;
            if (newPrice <= budget){
                if (newPrice>mostExpensive){
                    mostExpensive = newPrice;
                }
            }
        })
    })
